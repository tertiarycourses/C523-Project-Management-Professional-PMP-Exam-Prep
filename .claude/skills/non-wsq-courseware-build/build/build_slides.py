...
def txt(s,x,y,w,h,runs,align=PP_ALIGN.LEFT,anchor=MSO_ANCHOR.TOP,space=4):
    ...
    return tb
def fit_size(text,w_in,h_in,size,min_size=11,cpl_at_10pt=None,bold=False):
    """Shrink `size` until `text` is estimated to fit a w_in x h_in box.

    Arial averages ~0.5em per character, so a 10pt font fits roughly
    w_in*20 characters per line. Line height is ~1.22em. We step the font down
    until the wrapped line count fits the available height, floored at
    min_size so text never becomes unreadable.
    """
    t=str(text)
    if not t: return size
    # Calibrated against rendered output: Arial at 10pt fits ~17 chars/inch of
    # box width once word-wrap ragged-right waste is accounted for (a naive
    # 0.5em/char estimate of 20 is too optimistic and overflows). Line pitch is
    # ~1.30em including paragraph leading.
    cpl10 = cpl_at_10pt if cpl_at_10pt else w_in*17.0
    sz=size
    while sz>min_size:
        cpl=max(8.0,cpl10*10.0/sz)                 # chars per line at this size
        lines=_wrap_lines(t,cpl,w_in=w_in,size=sz,bold=bold)
        if lines*(sz*1.30/72.0) <= h_in: break
        sz-=1
    return sz
try:
    ...
    def _font(size,bold):
        ...
        if k not in _FONTS:
            ...
            _FONTS[k]=_IF.truetype(p,int(size*4))   # 4x for sub-point precision
        return _FONTS[k]
    def _text_w(t,size,bold):
        return _font(size,bold).getlength(str(t))/4.0/72.0   # -> inches
except Exception:                                   # PIL/Arial unavailable
    _text_w=None

def _wrap_lines(t,cpl,w_in=None,size=None,bold=False):
    """Count wrapped lines with greedy word wrap.

    When PIL and Arial are available this measures REAL glyph widths, which is
    the only reliable way to handle bold text and long unbreakable tokens such
    as 'artifacts/11-wbs-dictionary.md'. Falls back to a chars-per-line
    approximation otherwise.
    """
    words=str(t).split()
    if not words: return 1
    if _text_w is not None and w_in and size:
        lines=1; cur=""
        for w in words:
            trial=w if not cur else cur+" "+w
            if _text_w(trial,size,bold)<=w_in:
                cur=trial
            else:
                if cur: lines+=1
                cur=w
                while _text_w(cur,size,bold)>w_in and len(cur)>1:
                    # token wider than the line: it will hard-break
                    lo,hi=1,len(cur)
                    while lo<hi:
                        mid=(lo+hi+1)//2
                        if _text_w(cur[:mid],size,bold)<=w_in: lo=mid
                        else: hi=mid-1
                    cur=cur[lo:]; lines+=1
        return lines
    cpl=max(1,int(cpl)); lines=1; used=0
    for w in words:
...
def activity_overview(tag,title,desc,build,services,kicker):
    ...
    # The description box is 11.7in x 1.6in, ending at y=4.15 — clear of the
    # "You'll build" panel that starts at y=4.30.
    dsize=fit_size(desc,11.45,1.6,21,min_size=10)
    txt(s,Inches(0.85),Inches(2.55),Inches(11.7),Inches(1.6),[[(desc,dsize,_ink(),False)]])
    # Panel height is computed after the build text is measured (below) so a
    # 3-line build never overruns the panel onto the footer.
    # Measure against a slightly narrower width than the textbox: PowerPoint and
    # LibreOffice both reserve internal left/right insets (~0.1in each) that raw
    # glyph metrics don't know about, so measuring at the full 11.0in
    # under-counts lines and lets text overrun the panel.
    BW=10.75
    bsize=fit_size(build,BW,1.05,18,min_size=11,bold=True)
    _bl=_wrap_lines(build,0,w_in=BW,size=bsize,bold=True)
    _bh=_bl*(bsize*1.30/72.0)
    panel_h=max(2.0,(4.88+_bh+0.10+0.40)-4.30+0.18)
    rect(s,Inches(0.85),Inches(4.3),Inches(11.7),Inches(panel_h),_panel())
    txt(s,Inches(1.1),Inches(4.5),Inches(11),Inches(0.4),[[("You'll build",14,_acc(BLUE),True)]])
    # Fit the build text into the panel's 0.6in slot so it never overruns the
    # Tools line beneath it or spills past the panel onto the footer.
    # The build text renders BOLD, which is wider than regular Arial, so it gets
    # a reduced chars-per-line budget. It may legitimately need 3 lines, so the
    # "Tools:" row is placed below the text's ACTUAL measured height rather than
    # at a fixed offset — that is what previously caused the line collision.
    txt(s,Inches(1.1),Inches(4.88),Inches(11),Inches(_bh+0.1),[[(build,bsize,_ink(),True)]])
    ty=4.88+_bh+0.10
    txt(s,Inches(1.1),Inches(ty),Inches(11.2),Inches(0.4),[[("Tools:  ",13,_grey(),True),(services,13,_grey(),False)]]); footer(s); return s
def step_slide(kicker,act_title,n,total,text,cmd=""):
    ...
    ssize=fit_size(text,9.85,1.55,23,min_size=12)
    txt(s,Inches(2.55),Inches(2.4),Inches(10.1),Inches(1.6),[[(text,ssize,_ink(),False)]],anchor=MSO_ANCHOR.MIDDLE)
...
def test_slide(act_title,text,kicker):
    ...
    # The green panel spans y=2.30..4.90; the body starts at 3.30, so it has
    # 1.5in of room. Fit to that so verification text never spills below it.
    tsize=fit_size(text,10.75,1.5,18,min_size=10)
    txt(s,Inches(1.2),Inches(3.3),Inches(11),Inches(1.5),[[(text,tsize,_ink(),False)]]); footer(s); return s
