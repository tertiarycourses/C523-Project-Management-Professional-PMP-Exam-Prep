...
admin. Run **after** `/non-wsq-gdrive-push` — that publishes the files, this publishes
the links to them.
...
2. **Push the files first.** The links are read from Drive, so run
   `/non-wsq-gdrive-push` before this. A file that isn't on Drive can't be linked.
...
1. **Run the pusher** — the script lives in `.claude/scripts/` (project) or
   `~/.claude/skills/non-wsq-lms-push/`:
   ```bash
